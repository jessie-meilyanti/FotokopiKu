    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-4">
        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
            <div class="flex-1 min-w-0">
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white truncate">Pesanan #{{ $order->id }}</h1>
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3 sm:flex-shrink-0">
                <a href="{{ route('orders.invoice', $order) }}" class="w-full sm:w-auto">
                    <x-button color="indigo" class="w-full sm:w-auto">Unduh Invoice</x-button>
                </a>
                ...
                @if(!in_array($order->status, ['completed','cancelled']))
                    <form action="{{ route('orders.cancel', $order) }}" method="POST" onsubmit="return confirm('Batalkan pesanan ini?');" class="w-full sm:w-auto">
                        @csrf
                        <x-button type="submit" color="red" class="w-full sm:w-auto">Batalkan Pesanan</x-button>
                    </form>
                @endif
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100/70 dark:border-gray-700 p-3 sm:p-4">
            ...
            <div class="divide-y divide-gray-100 dark:divide-gray-700">
                @foreach ($order->items as $item)
                    ...
                    <div class="py-3 flex flex-col sm:flex-row sm:items-center gap-3">
                        <img src="{{ $thumb }}" alt="{{ $item->product->name }}" class="w-16 h-16 rounded-lg object-cover border border-gray-100 dark:border-gray-700 flex-shrink-0">
                        <div class="flex-1 min-w-0">
                            <div class="font-medium text-gray-900 dark:text-white truncate">{{ $item->product->name }}</div>
        ...
        @if($order->payment_method === 'Transfer' && $order->payment_status !== 'paid')
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100/70 dark:border-gray-700 p-3 sm:p-4 space-y-3">
                <h2 class="font-semibold text-gray-900 dark:text-white">Upload Bukti Transfer</h2>
                    ...
                    <x-button type="submit" color="indigo" class="w-full sm:w-auto">Kirim Bukti</x-button>
                </form>
            </div>
        @endif
